...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    combs = list(itertools.combinations([~x for x in literals], 2))
    l = []
    for x,y in combs:
        l.append(x | y)
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None, sensorModel: Callable = None, successorAxioms: Callable = None) -> Expr:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    pacphysics_sentences.extend([PropSymbolExpr(wall_str, x, y) >> (~PropSymbolExpr(pacman_str, x, y, time=t)) for x,y in all_coords])
    ...
    if successorAxioms and t > 0: pacphysics_sentences.append(successorAxioms(t, walls_grid, non_outer_wall_coords))
...
def checkLocationSatisfiability(x1_y1: Tuple[int, int], x0_y0: Tuple[int, int], action0, action1, problem):
    ...
    res = []
    res.append(findModel(conjoin(*KB)))
    KB[6] = ~KB[6]
    res.append(findModel(conjoin(*KB)))
...
def positionLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        tmp = findModel(conjoin(KB) & PropSymbolExpr(pacman_str, xg, yg, time=t))
        if tmp:
...
def foodLogicPlan(problem) -> List:
    ...
    for t in range(50):
        print(t)
        KB.append(exactlyOne([PropSymbolExpr(pacman_str, x, y, time=t) for x,y in non_wall_coords]))
        goalcheck = [~PropSymbolExpr(food_str, x, y, time=t) for x,y in food]
        tmp = findModel(conjoin(KB + goalcheck))
        if tmp:
            return extractActionSequence(tmp, actions)
        KB.append(exactlyOne([PropSymbolExpr(action, time=t) for action in actions]))
...
def localization(problem, agent) -> Generator:
    ...
    KB.extend([~PropSymbolExpr(wall_str, x, y) for x,y in all_coords if (x,y) not in walls_set])
