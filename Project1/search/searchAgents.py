...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        return self.__startState # (self.startingPosition, self.visited_corners)
    ...
    def isGoalState(self, state: Any):
        ...
        """
        "*** YOUR CODE HERE ***"
        # goal state when visited_corners = (True, True, True, True)
    ...
    def getSuccessors(self, state: Any):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            dx,dy = Actions.directionToVector(action)
            visited_corners = list(state[1]) # visited_corners tuple => list because it may change values. Example current visited_corners = [True, False, False, True]
            next_move = (nextx, nexty) = (int(x + dx), int(y + dy)) # next move
            if not self.walls[nextx][nexty]:  # if next move doesn't hit the wall
                if next_move in self.corners: # if next move is in a corner
                    if next_move == self.corners[0]:    # next_move == (1,1) 
                        visited_corners[0] = True
                    elif next_move == self.corners[1]:  # next_move == (1,top) 
                        visited_corners[1] = True
                    elif next_move == self.corners[2]:  # next_move == (right, 1)
                        visited_corners[2] = True
                    elif next_move == self.corners[3]:  # next_move == (right, top)
                        visited_corners[3] = True    

                next_state = (next_move, tuple(visited_corners)) # visited_corners list => tuple, otherwise it bugs.. next_state = (next_move, visited)
                cost = 1                                         # cost = 1 because of Pacman 
                successors.append((next_state, action, cost))    # getSuccessors => (next_state, action, stepCost)
...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    for i in range(0, len(problem.corners)): # 0 <= i < 4
        corner = problem.corners[i]
        isVisited = state[1][i] # a state contains (position, tuple: visited_corners)
        # distance from current (x, y) to a corner using Manhattan Distance 
        distance_to_corner = util.manhattanDistance(current, corner) 

        # Finds the max distance out of all corners and returns it
        if not isVisited and (distance_to_corner > max_distance):
            max_distance = distance_to_corner
